...
dotenv_path = join(dirname(__file__), 'data',
                   'master', 'jenkins_conf', env_file)
list_of_ENV = {
...
def wrap_input(env, env_value):
    ...
    if '_PSW' in env:
        ...
        if '_EMAIL' in env and user_value and not validate_email_syntax(
                user_value) or '_URL' in env and user_value and not validators.url(
                user_value):
            return wrap_input(env, env_value)
    try:
        if not user_value:
...
if __name__ == '__main__':
    ...
    try:
        for env_key, env_value in list_of_ENV.items():
            valide_value = wrap_input(env_key, env_value)
            set_variable_in_file(env_key, valide_value)
    except Exception as e:
        print(e)
        for env_key, env_value in list_of_ENV.items():
